...
category = st.sidebar.selectbox("Category", ["Food","Travel","Shopping","Bills","Other"], key="cat")
amount = st.sidebar.number_input("Amount", min_value=0.0, key="amt")
note = st.sidebar.text_input("Note", key="note")
type_ = st.sidebar.selectbox("Type", ["expense","income"], key="type")
...
if st.sidebar.button("Add"):
    ...
    # ✅ fast reset (ONLY fields)
    st.session_state.cat = "Food"
    st.session_state.amt = 0.0
    st.session_state.note = ""
    st.session_state.type = "expense"

    st.rerun()   # fast refresh
